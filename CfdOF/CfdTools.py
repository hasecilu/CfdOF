...
FOAM_DIR_DEFAULTS = {
    ...
    "Linux": [
        ...
        "/opt/openfoam*",
        "/opt/openfoam-dev",  # Foundation official packages
        "~/openfoam/OpenFOAM-v*",
        "~/OpenFOAM/OpenFOAM-*.*",
        "~/OpenFOAM/OpenFOAM-dev",
    ],  # Typical self-built locations
...
def getOutputPath(analysis):
    ...
    if not os.path.isabs(output_path):
        if not FreeCAD.ActiveDocument.FileName:
            raise RuntimeError(
                "The output directory is specified as a path relative to the current file's location; "
...
def getDynamicMeshAdaptation(mesh_object):
    ...
    for i in mesh_object.Group:
        if i.Name.startswith("DynamicMeshInterfaceRefinement") or i.Name.startswith(
            "DynamicMeshShockRefinement"
        ):
...
def setCompSolid(vobj):
    ...
    for obj in doc.Objects:
        if hasattr(obj, "Proxy") and isinstance(
            obj.Proxy, BOPTools.SplitFeatures.FeatureBooleanFragments
        ):
...
def startDocker():
    ...
    if docker_container.container_id == None:
        ...
        if docker_container.container_id != None:
            print(
                "Docker image {} started. ID = {}".format(
                    docker_container.image_name, docker_container.container_id
                )
            )
            return 0
        else:
            print(
                f"Unable to start {docker_container.docker_cmd} container. Refer to README for installation instructions."
            )
            return 1
    else:
...
def getFoamRuntime():
    ...
    if platform.system() == "Windows":
        if os.path.exists(
            os.path.join(installation_path, "msys64", "home", "ofuser", ".blueCFDCore")
        ):
            runtime = "BlueCFD"
        elif os.path.exists(
            os.path.join(installation_path, "..", "msys64", "home", "ofuser", ".blueCFDCore")
    ...
    if not runtime:
        raise IOError(
            "The directory {} is not a recognised OpenFOAM installation".format(installation_path)
        )
...
def fromWindowsPath(p):
    ...
    pp = tail.replace("\\", "/")
    if (
        getFoamRuntime() == "MinGW"
        or getFoamRuntime() == "BlueCFD"
        or getFoamRuntime() == "BlueCFD2"
    ):
        ...
        if os.path.isabs(p):
            ...
            try:
                if os.path.commonpath((os.path.normpath(p), homepath)) == homepath:
                    ...
                    raise ValueError(
                        "The path {} is not inside the users's home directory.".format(p)
                    )
            except ValueError:
                cfdError(
...
def makeRunCommand(cmd, dir=None, source_env=True):
    ...
    if getFoamRuntime() == "PosixDocker":
        ...
        if dir:
            cd = cd.replace(
                FreeCAD.ParamGet(prefs).GetString("DefaultOutputPath", ""), "/tmp"
            ).replace("\\", "/")
    ...
    if getFoamRuntime() == "PosixDocker":
        ...
        if docker_container.output_path_used != FreeCAD.ParamGet(prefs).GetString(
            "DefaultOutputPath", ""
        ):
        ...
        if (
            ...
            and FreeCAD.ParamGet(prefs).GetString("DefaultOutputPath", "")[:5] == "\\\\wsl"
            and cmd[:5] == "./All"
        ):
            cmd = "chmod 744 {0} && {0}".format(
                cmd
            )  # If using windows wsl$ output directory, need to make the command executable
    ...
    if getFoamRuntime() == "WindowsDocker":
        ...
        cmdline = [
            ...
            + (cd + cmd)
            .replace("$", "`$")
            .replace('"', '\\`"')  # Escape quotes for powershell and also cmdline to bash
            + "' -l ofuser\"",
        ]
        return cmdline
    elif getFoamRuntime() == "BashWSL":
        ...
        with os.scandir("{}".format(inst_path)) as dirs:
            for dir in dirs:
                if dir.is_dir() and dir.name.startswith("OpenFOAM-"):
                    destdir1 = os.path.join(
                        inst_path, dir.name, "platforms\\mingw_w64GccDPInt32Opt\\bin"
                    )
                if dir.is_dir() and dir.name.startswith("ofuser-of"):
                    destdir2 = os.path.join(
                        inst_path, dir.name, "platforms\\mingw_w64GccDPInt32Opt\\bin"
                    )
        if not destdir1 or not destdir2:
            cfdError(
                "Unable to find directories 'OpenFOAM-*' and 'ofuser-of*' in path {}. "
        ...
        try:
            ...
            cfdError(
                "Unable to copy file {} from directory {} to {} and {}: {}\n"
                "Try running FreeCAD again with administrator privileges, or copy the file manually.".format(
                    file, srcdir, destdir1, destdir2, str(err)
                )
            )
        ...
        cmdline = [
            "{}\\msys64\\usr\\bin\\bash".format(short_bluecfd_path),
            "--login",
            "-O",
            "expand_aliases",
            "-c",
...
def startFoamApplication(
    cmd, case, log_name="", finished_hook=None, stdout_hook=None, stderr_hook=None
):
    """
    ...
    proc = CfdConsoleProcess(
        finished_hook=finished_hook, stdout_hook=stdout_hook, stderr_hook=stderr_hook
    )
    if logFile:
...
def checkCfdDependencies(msgFn):
    ...
    message = ""
    FreeCAD.Console.PrintMessage(translate("Console", "Checking CFD workbench dependencies...\n"))
    ...
    if major_ver < FC_MAJOR_VER_REQUIRED or (
        ...
        msgFn(
            "FreeCAD version (currently {}.{}.{} ({})) must be at least {}.{}.{} ({})".format(
                int(ver[0]),
                minor_ver,
                patch_ver,
                gitver,
    ...
    try:
        ...
        msgFn(
            "System: {}\nRuntime: {}\nOpenFOAM directory: {}".format(
                platform.system(),
                getFoamRuntime(),
                foam_dir if len(foam_dir) else "(system installation)",
            )
        )
    except IOError as e:
        ...
        if foam_dir is None:
            msgFn(
                "OpenFOAM installation path not set and OpenFOAM environment neither pre-loaded before "
            ...
            try:
                        ...
                        if foam_ver >= 1000:  # Plus version
                            if foam_ver < MIN_OCFD_VERSION:
                                msgFn(
                                    ...
                                    + " is outdated:\n"
                                    + "Minimum version "
                                    + str(MIN_OCFD_VERSION)
                                    + " or "
                                    + str(MIN_FOUNDATION_VERSION)
                ...
                if getFoamRuntime() != "MinGW" and getFoamRuntime() != "PosixDocker":
                    try:
                        ...
                        msgFn(
                            "OpenFOAM installation does not include 'wmake'. "
                ...
                try:
                    ...
                    msgFn("MPI was not found. " + "Parallel execution will not be possible.")
                ...
                try:
                    ...
                    if (
                        not cfmesh_ver
                        or len(cfmesh_ver) != 2
                        or int(cfmesh_ver[0]) < CF_MAJOR_VER_REQUIRED
                        or (
                            int(cfmesh_ver[0]) == CF_MAJOR_VER_REQUIRED
                            and int(cfmesh_ver[1]) < CF_MINOR_VER_REQUIRED
                        )
                    ):
                        msgFn(
                            "cfMesh-CfdOF version {}.{} required".format(
                                CF_MAJOR_VER_REQUIRED, CF_MINOR_VER_REQUIRED
                            )
                        )
                except subprocess.CalledProcessError:
                ...
                try:
                    ...
                    if (
                        not hisa_ver
                        or len(hisa_ver) != 3
                        or int(hisa_ver[0]) < HISA_MAJOR_VER_REQUIRED
                        or (
                            int(hisa_ver[0]) == HISA_MAJOR_VER_REQUIRED
                            and (
                                int(hisa_ver[1]) < HISA_MINOR_VER_REQUIRED
                                or (
                                    int(hisa_ver[1]) == HISA_MINOR_VER_REQUIRED
                                    and int(hisa_ver[2]) < HISA_PATCH_VER_REQUIRED
                                )
                            )
                        )
                    ):
                        msgFn(
                            "HiSA version {}.{}.{} required".format(
                                HISA_MAJOR_VER_REQUIRED,
...
def isSameGeometry(shape1, shape2):
    ...
    if len(shape1.Vertexes) == len(shape2.Vertexes) and len(shape1.Vertexes) > 0:
        ...
        if hasattr(shape1, "CenterOfMass") and hasattr(shape2, "CenterOfMass"):
            if (
                not floatEqual(shape1.CenterOfMass[0], shape2.CenterOfMass[0])
                or not floatEqual(shape1.CenterOfMass[1], shape2.CenterOfMass[1])
                or not floatEqual(shape1.CenterOfMass[2], shape2.CenterOfMass[2])
            ):
                return False
        ...
        for vs1 in shape1.Vertexes:
            for vs2 in shape2.Vertexes:
                if (
                    floatEqual(vs1.X, vs2.X)
                    and floatEqual(vs1.Y, vs2.Y)
                    and floatEqual(vs1.Z, vs2.Z)
                ):
                    same_Vertexes += 1
...
def findElementInShape(a_shape, an_element):
    ...
    if ele_st == "Solid" or ele_st == "CompSolid":
        ...
        FreeCAD.Console.PrintError(
            "Solid " + str(an_element) + " not found in: " + str(a_shape) + "\n"
        )
        ...
        try:
            if rr.startswith("Solid"):  # getElement doesn't work with solids for some reason
                ...
                if ff is None:
                    if raise_error:
                        raise RuntimeError(
                            "Face '{}:{}' was not found - geometry may have changed".format(
                                r[0].Name, rr
                            )
                        )
                else:
            ...
            if raise_error:
                raise RuntimeError(
                    "Face '{}:{}' was not found - geometry may have changed".format(r[0].Name, r[1])
                )
    return f
...
class DockerContainer:
    ...
    def start_container(self):
        ...
        if container_ls != None:
            print(
                "Docker container {} running but not started by CfdOF - it may not be configured correctly - stopping container.".format(
                    container_ls
                )
            )
        ...
        if (
            "podman" in self.docker_cmd
        ):  # podman runs without root privileges, so the user in the container can be root.
            usr_str = "-u 0:0"
        else:
            if platform.system() == "Windows":
        ...
        if "podman" in self.docker_cmd:
            podman_opts = (
                "--security-opt label=disable"  # Allows /tmp to be mounted to the podman container
            )

        cmd = "{0} run -t -d {1} {2} -v {3}:/tmp {4}".format(
            self.docker_cmd, podman_opts, usr_str, output_path, self.image_name
        )
    ...
    def query_docker_container_ls(self):
        ...
        while proc.canReadLine():
            ...
            if len(cnt_lst) > 0 and cnt_lst[1].replace(":latest", "").replace(
                "docker.io/", ""
            ) == self.image_name.replace(":latest", "").replace("docker.io/", ""):
                return cnt_lst[0]
        return None
