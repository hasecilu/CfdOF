...
class CfdMeshTools:
    def __init__(self, cart_mesh_obj):
        ...
        if self.clmax <= 0.0:
            ...
            cl_bound_mag = math.sqrt(
                shape.BoundBox.XLength**2 + shape.BoundBox.YLength**2 + shape.BoundBox.ZLength**2
            )
            cl_bound_min = min(
                min(shape.BoundBox.XLength, shape.BoundBox.YLength), shape.BoundBox.ZLength
            )
            self.clmax = min(
                0.02 * cl_bound_mag, 0.4 * cl_bound_min
            )  # Always in internal format, i.e. mm
    ...
    def processExtrusions(self):
        ...
        all_extrusion_objs = (
            other_extrusion_objs + twoD_extrusion_objs
        )  # Ensure 2D extrusion happens last
        ...
        for extrusion_obj in all_extrusion_objs:
            ...
            if (
                extrusion_obj.ExtrusionType == "2DPlanar"
                or extrusion_obj.ExtrusionType == "2DWedge"
            ):
            ...
            if not efl:
                raise RuntimeError(
                    "Extrusion patch for '{}' could not be found in the shape being meshed.".format(
                        extrusion_obj.Label
                    )
                )
            ...
            this_extrusion_settings["AxisPoint"] = tuple(
                Units.Quantity(p, Units.Length).getValueAs("m")
                for p in extrusion_obj.ExtrusionAxisPoint
            )
            ...
            if len(extrusion_shape.Faces) > 0:
                in_plane_vector = (
                    extrusion_shape.Faces[0].CenterOfMass - extrusion_obj.ExtrusionAxisPoint
                )
    ...
    def getFilePaths(self, output_dir):
        ...
        if self.mesh_obj.MeshUtility == "gmsh":
            ...
            self.temp_file_geo = os.path.join(
                self.constantDir, "triSurface", self.part_obj.Name + "_Geometry.stl"
            )
    ...
    def processRefinements(self):
        ...
        mesh_face_list = list(
            zip(self.mesh_obj.Part.Shape.Faces, range(len(self.mesh_obj.Part.Shape.Faces)))
        )
        ...
        for bc_id, bc_obj in enumerate(bc_group):
            for ri, ref in enumerate(bc_obj.ShapeRefs):
                try:
                    ...
                    raise RuntimeError(
                        "Error processing boundary condition {}: {}".format(bc_obj.Label, str(re))
                    )
            ...
            if prev_k >= 0:
                ...
                CfdTools.cfdWarning(
                    ...
                    "boundary '{}' reference {}:{} - ignoring duplicate\n".format(
                        bc.Label,
                        bc.ShapeRefs[ri][0].Name,
                        bc.ShapeRefs[ri][1][si],
                        bc2.Label,
                        bc.ShapeRefs[ri][0].Name,
                        bc.ShapeRefs[ri][1][si],
                    )
        ...
        for mr_id, mr_obj in enumerate(mr_objs):
            if (
                ...
                or (
                    ...
                    and mr_obj.NumberLayers > 0
                )
                or (self.mesh_obj.MeshUtility == "snappyHexMesh" and not mr_obj.Internal)
            ):
                for ri, r in enumerate(mr_obj.ShapeRefs):
                    try:
                        ...
                        raise RuntimeError(
                            "Error processing mesh refinement {}: {}".format(mr_obj.Label, str(re))
                        )
                    for si, s in enumerate(bf):
        ...
        if self.mesh_obj.MeshUtility == "gmsh":
            ...
            mesh_vertices_list = list(
                zip(
                    self.mesh_obj.Part.Shape.Vertexes, range(len(self.mesh_obj.Part.Shape.Vertexes))
                )
            )
            ...
            if bc_obj.BoundaryType == "baffle":
                ...
                for ri, mr in enumerate(mr_match_per_baffle_ref):
                    for si, mri in enumerate(mr_match_per_baffle_ref[ri]):
                        baffle_patch_refs[mri + 1].append(
                            (bc_obj.ShapeRefs[ri][0], (bc_obj.ShapeRefs[ri][1][si],))
                        )
                ...
                for ri, refs in enumerate(baffle_patch_refs):
                    try:
                        ...
                        raise RuntimeError(
                            "Error processing baffle {}: {}".format(bc_obj.Label, str(re))
                        )
                    ...
                    if shape:
                        ...
                        writeSurfaceMeshFromShape(
                            shape, self.triSurfaceDir, solid_name, self.mesh_obj
                        )

                        if (
                            ri > 0
                        ):  # The parts of the baffle corresponding to a surface mesh region obj
                            mr_obj = mr_objs[ri - 1]
        ...
        for mr_id, mr_obj in enumerate(mr_objs):
            ...
            if self.mesh_obj.MeshUtility == "gmsh":
                ...
                for m in bc_matches:
                    bc_match_per_mr_ref[m[1][1]][m[1][2]] = (
                        -2 if bc_group[m[0][0]].BoundaryType == "baffle" else m[0][0]
                    )
                    ...
                    for si, bci in enumerate(m):
                        if bci > -2:
                            mr_patch_refs[bci + 1].append(
                                (mr_obj.ShapeRefs[ri][0], (mr_obj.ShapeRefs[ri][1][si],))
                            )
                ...
                for bi in range(len(mr_patch_refs)):
                    if len(mr_patch_refs[bi]) and not mr_obj.Extrusion:
                        ...
                        try:
                            ...
                            raise RuntimeError(
                                "Error processing mesh refinement region {}: {}".format(
                                    mr_obj.Label, str(re)
                                )
                            )
                        if shape:
                            writeSurfaceMeshFromShape(
                                shape, self.triSurfaceDir, mr_patch_name, self.mesh_obj
                            )
                        ...
                        if self.mesh_obj.MeshUtility == "cfMesh":
                            if not Internal:
                                cf_settings["MeshRegions"][mr_patch_name] = {
                                    "RefinementLevel": refinement_level,
                                    "RefinementThickness": self.scale
            ...
            if (
                (
                    self.mesh_obj.MeshUtility == "cfMesh"
                    or self.mesh_obj.MeshUtility == "snappyHexMesh"
                )
                and mr_obj.NumberLayers > 0
                and not Internal
                and not mr_obj.Extrusion
            ):
    ...
    def automaticInsidePointDetect(self):
        ...
        if (
            step_size * error_safety_factor >= bound_box.XLength
                ...
                for k in range(len(self.patch_faces)):
                    for l in range(len(self.patch_faces[k])):
                        ...
                        if len(patch_faces):
                            ...
                            with open(
                                os.path.join(self.triSurfaceDir, patch_name + ".stl"), "r"
                            ) as fid2:
                                for ln in fid2:
                                    fid.write(ln)
    ...
    def loadSurfMesh(self):
        if not self.error:
            ...
            fem_mesh_obj = FreeCAD.ActiveDocument.addObject(
                "Fem::FemMeshObject", self.mesh_obj.Name + "SurfaceMesh"
            )
    ...
    def writeMeshCase(self):
        ...
        # SnappyHexMesh settings
            ...
            snappy_settings["EdgeRefinementLevel"] = CfdTools.relLenToRefinementLevel(
                self.mesh_obj.EdgeRefinement
            )
            snappy_settings["PointInMesh"] = {"x": inside_x, "y": inside_y, "z": inside_z}
        ...
        # GMSH settings
            ...
            sols = (
                "".join((str(n + 1) + ", ") for n in range(len(self.mesh_obj.Part.Shape.Solids)))
            ).rstrip(", ")
            ...
            for k in range(len(self.patch_faces)):
                for l in range(len(self.patch_faces[k])):
                    ...
                    patch_name = self.patch_names[k][l]
                    if len(patch_faces):
                        self.gmsh_settings["BoundaryFaceMap"][patch_name] = ", ".join(
        ...
        if (
            self.mesh_obj.MeshUtility == "cfMesh"
            and len(self.cf_settings["MeshRegions"]) > 0
            or self.mesh_obj.MeshUtility == "snappyHexMesh"
            and len(self.snappy_settings["MeshRegions"]) > 0
        ):
...
def writeSurfaceMeshFromShape(shape, path, name, mesh_obj):
    ...
    if use_gmsh:
        with tempfile.TemporaryDirectory() as tmpdirname:
            ...
            TemplateBuilder(
                tmpdirname,
                os.path.join(CfdTools.getModulePath(), "Data", "Templates", "surfaceMesh"),
                settings,
            )
