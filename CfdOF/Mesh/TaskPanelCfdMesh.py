...
class TaskPanelCfdMesh:
    ...
    def __init__(self, obj):
        ...
        self.form = FreeCADGui.PySideUic.loadUi(
            os.path.join(CfdTools.getModulePath(), "Gui", "TaskPanelCfdMesh.ui")
        )
    ...
    def load(self):
        ...
        index_utility = CfdTools.indexOrDefault(
            list(zip(CfdMesh.MESHERS, CfdMesh.DIMENSION, CfdMesh.DUAL_CONVERSION)),
            (
                self.mesh_obj.MeshUtility,
                self.mesh_obj.ElementDimension,
                self.mesh_obj.ConvertToDualMesh,
            ),
            0,
        )
    ...
    def updateUI(self):
        ...
        self.form.pb_load_mesh.setEnabled(
            os.path.exists(os.path.join(case_path, "surfaceMesh.vtk"))
        )
        self.form.pb_check_mesh.setEnabled(
            os.path.exists(os.path.join(case_path, "surfaceMesh.vtk"))
        )
    ...
    def consoleMessage(self, message="", colour_type=None, timed=True):
        if timed:
            self.console_message_cart += '<font color="{}">{:4.1f}:</font> '.format(
                CfdTools.getColour("Logging"), time.time() - self.Start
            )
        if colour_type:
            self.console_message_cart += '<font color="{}">{}</font><br>'.format(
    ...
    def writeMesh(self):
        ...
        FreeCADGui.doCommand(
            "FreeCAD.ActiveDocument." + self.mesh_obj.Name + ".Proxy.cart_mesh = cart_mesh"
        )
        ...
        try:
            ...
            print(
                "Part to mesh:\n  Name: "
                + cart_mesh.part_obj.Name
                + ", Label: "
                + cart_mesh.part_obj.Label
                + ", ShapeType: "
    ...
    def checkMeshClicked(self):
        ...
        try:
            ...
            FreeCADGui.doCommand(
                "cmd = CfdTools.makeRunCommand('checkMesh -meshQuality', cart_mesh.meshCaseDir)"
            )
    ...
    def killMeshProcess(self):
        if CfdTools.getFoamRuntime() == "PosixDocker":
            FreeCADGui.doCommand("from CfdOF import CfdConsoleProcess")
            FreeCADGui.doCommand(
                "cmd = CfdTools.makeRunCommand('killall Allmesh', None, source_env=False)"
            )
            FreeCADGui.doCommand("env_vars = CfdTools.getRunEnvironment()")
            FreeCADGui.doCommand(
                "kill_process = CfdConsoleProcess.CfdConsoleProcess()\n"
    ...
    def pbLoadMeshClicked(self):
        ...
        self.consoleMessage(
            "Triangulated representation of the surface mesh is shown - ", timed=False
        )
