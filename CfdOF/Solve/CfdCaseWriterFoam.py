...
class CfdCaseWriterFoam:
    def __init__(self, analysis_obj):
        ...
        if not self.material_objs:
            raise RuntimeError(
                "No material properties were found in analysis " + analysis_obj.Label
            )
        ...
        if not self.initial_conditions:
            raise RuntimeError(
                "No initial conditions object was found in analysis " + analysis_obj.Label
            )
    ...
    def writeCase(self):
        ...
        self.mesh_file_name = os.path.join(self.case_folder, self.solver_obj.InputCaseName, ".unv")
        ...
        for i, l in enumerate(bc_labels):
            for j in range(i + 1, len(bc_labels)):
                if bc_labels[j] == l:
                    raise ValueError(
                        "Boundary condition label '" + bc_labels[i] + "' is duplicated"
                    )
        ...
        self.settings = {
            ...
            "reportingFunctions": dict(
                (fo.Label, CfdTools.propsToDict(fo)) for fo in self.reporting_functions
            ),
            "reportingFunctionsEnabled": False,
            "scalarTransportFunctions": dict(
                (st.Label, CfdTools.propsToDict(st)) for st in self.scalar_transport_objs
            ),
            ...
            "initialisationZones": {
                o.Label: CfdTools.propsToDict(o) for o in self.initialisation_zone_objs
            },
            "initialisationZonesPresent": len(self.initialisation_zone_objs) > 0,
            "zones": {
                o.Label: {"PartNameList": tuple(r[0].Name for r in o.ShapeRefs)}
            ...
            "meshDir": os.path.relpath(
                os.path.join(self.working_dir, self.mesh_obj.CaseName), self.case_folder
            ),
    ...
    def getSolverName(self):
        ...
        if self.physics_model.Phase == "Single":
            if len(self.material_objs) == 1:
                if self.physics_model.Flow == "Isothermal":
                    ...
                    raise RuntimeError(
                        self.physics_model.Flow + " flow model currently not supported."
                    )
            else:
                raise RuntimeError(
                    "Only one material object may be present for single phase simulation."
                )
            ...
            if self.physics_model.Time == "Transient":
                if self.physics_model.Flow == "Isothermal":
                    if len(self.material_objs) == 2:
                        ...
                        raise RuntimeError(
                            "At least two material objects must be present for free surface simulation."
                        )
                else:
                    raise RuntimeError(
                        "Only isothermal analysis is currently supported for free surface flow simulation."
                    )
            else:
                raise RuntimeError(
                    "Only transient analysis is supported for free surface flow simulation."
                )
        else:
        ...
        if solver is None:
            raise RuntimeError(
                "No solver is supported to handle the selected physics with {} phases.".format(
    ...
    def processFluidProperties(self):
        ...
        for material_obj in self.material_objs:
            ...
            if ((flow_type == "Isothermal") != (mat_type == "Isothermal")) or (
                ...
                raise ValueError(
                    "The material type of object '{}' is not compatible with the selected flow physics. "
            ...
            if "DynamicViscosity" in mp:
                if self.physics_model.Turbulence == "Inviscid":
                    ...
                    mp["DynamicViscosity"] = (
                        Units.Quantity(mp["DynamicViscosity"]).getValueAs("kg/m/s").Value
                    )
            ...
            if "SutherlandTemperature" in mp:
                mp["SutherlandTemperature"] = (
                    Units.Quantity(mp["SutherlandTemperature"]).getValueAs("K").Value
                )
                if "SutherlandRefViscosity" in mp and "SutherlandRefTemperature" in mp:
                    ...
                    mp["SutherlandConstant"] = (
                        mu0 / T0 ** (3.0 / 2) * (T0 + mp["SutherlandTemperature"])
                    )
            for k in mp:
                if k.endswith("Polynomial"):
                    ...
                    for i in range(len(poly)):
                        try:
                            ...
                            raise ValueError(
                                "Invalid coefficient {} in polynomial coefficient {}".format(
                                    poly[i], k
                                )
                            )
        ...
        for bc_name in bc_names:
            ...
            if not bc["VelocityIsCartesian"]:
                ...
                if not face[0]:
                    raise RuntimeError(
                        str(
                            "No face specified for velocity direction in boundary '" + bc_name + "'"
                        )
                    )
                ...
                try:
                    ...
                    if bc["DirectionFace"]:
                        raise RuntimeError(
                            ...
                            + ", specified for velocity direction in boundary '"
                            + bc_name
                        ...
                        raise RuntimeError(
                            str(
                                "No face specified for velocity direction in boundary '"
                                + bc_name
                                + "'"
                            )
                        )
                ...
                for i, m in enumerate(settings["fluidProperties"]):
                    ...
                    if i == len(settings["fluidProperties"]) - 1:
                        ...
                        alpha = Units.Quantity(
                            bc.get("VolumeFractions", {}).get(alpha_name, "0")
                        ).Value
    ...
    def processReferenceFrames(self):
        if self.settings["solver"]["SolverName"] == "SRFSimpleFoam":
            self.settings["physics"]["SRFModelCoR"] = tuple(
                p for p in self.settings["physics"]["SRFModelCoR"]
            )
            self.settings["physics"]["SRFModelAxis"] = tuple(
                p for p in self.settings["physics"]["SRFModelAxis"]
            )
    ...
    def processInitialConditions(self):
        ...
        if settings["solver"]["SolverName"] in ["interFoam", "multiphaseInterFoam"]:
            ...
            for i, m in enumerate(settings["fluidProperties"]):
                ...
                if i == len(settings["fluidProperties"]) - 1:
                    ...
                    alpha = Units.Quantity(
                        initial_values.get("VolumeFractions", {}).get(alpha_name, "0")
                    ).Value
                    alphas_new[alpha_name] = alpha
                    sum_alpha += alpha
        ...
        if initial_values["PotentialFlow"]:
            if settings["solver"]["SolverName"] in ["SRFSimpleFoam"]:
                raise RuntimeError(
                    "Selected solver does not support potential flow velocity initialisation."
                )
        if initial_values["PotentialFlowP"]:
            if settings["solver"]["SolverName"] not in [
                "simpleFoam",
                "porousSimpleFoam",
                "pimpleFoam",
                "hisa",
                ...
                raise RuntimeError(
                    "Selected solver does not support potential flow pressure initialisation."
                )
        ...
        if initial_values["UseInletUValues"]:
            if initial_values["BoundaryU"]:
                ...
                if (
                    ...
                    or inlet_bc["BoundarySubType"] == "farField"
                ):
                    ...
                    raise RuntimeError(
                        "Boundary type not appropriate to determine initial velocity."
                    )
            else:
        ...
        if initial_values["UseOutletPValue"]:
            if initial_values["BoundaryP"]:
                ...
                if (
                    ...
                    or outlet_bc["BoundarySubType"] == "totalPressureOpening"
                    or outlet_bc["BoundarySubType"] == "totalPressureInlet"
                    or outlet_bc["BoundarySubType"] == "staticPressureInlet"
                    or outlet_bc["BoundarySubType"] == "farField"
                ):
        ...
        if physics["Flow"] != "Isothermal" and initial_values["UseInletTemperatureValue"]:
            ...
            if inlet_bc["BoundaryType"] == "inlet":
                if inlet_bc["ThermalBoundaryType"] == "fixedValue":
                    ...
                    raise RuntimeError(
                        "Inlet type not appropriate to determine initial temperature"
                    )
            ...
            if initial_values["UseInletTurbulenceValues"]:
                if initial_values["BoundaryTurb"]:
                    ...
                    if inlet_bc["TurbulenceInletSpecification"] == "TKEAndSpecDissipationRate":
                        ...
                        inlet_bc["TurbulenceInletSpecification"]
                        ...
                        if (
                            ...
                            Uin = (
                                inlet_bc["Ux"] ** 2 + inlet_bc["Uy"] ** 2 + inlet_bc["Uz"] ** 2
                            ) ** 0.5

                            # Turb Intensity (or Tu) and length scale
                            I = (
                                inlet_bc["TurbulenceIntensityPercentage"] / 100.0
                    ...
                    raise RuntimeError(
                        "No boundary selected to copy initial turbulence values from."
                    )
    ...
    def processReportingFunctions(self):
        ...
        for name in settings["reportingFunctions"]:
            ...
            rf["CentreOfRotation"] = tuple(
                Units.Quantity(p, Units.Length).getValueAs("m") for p in rf["CentreOfRotation"]
            )
    ...
    def processScalarTransportFunctions(self):
        ...
        for name in settings["scalarTransportFunctions"]:
            ...
            if settings["solver"]["SolverName"] in ["simpleFoam", "porousSimpleFoam", "pimpleFoam"]:
                stf["InjectionRate"] = (
                    stf["InjectionRate"] / settings["fluidProperties"][0]["Density"]
                )
                stf["DiffusivityFixedValue"] = (
                    stf["DiffusivityFixedValue"] / settings["fluidProperties"][0]["Density"]
                )
    ...
    def processDynamicMeshRefinement(self):
        ...
        if isinstance(
            self.dynamic_mesh_refinement_obj.Proxy,
            CfdDynamicMeshRefinement.CfdDynamicMeshShockRefinement,
        ):
    ...
    def exportZoneStlSurfaces(self):
        for zo in self.zone_objs:
            for r in zo.ShapeRefs:
                path = os.path.join(
                    self.working_dir, self.solver_obj.InputCaseName, "constant", "triSurface"
                )
    ...
    def processInitialisationZoneProperties(self):
        ...
        for zone_name in settings["initialisationZones"]:
            ...
            if settings["solver"]["SolverName"] in [
                ...
                if "Pressure" in z:
                    z["KinematicPressure"] = (
                        z["Pressure"] / settings["fluidProperties"][0]["Density"]
                    )
    ...
    def porousBafflesPresent(self):
        for b in self.bc_group:
            if b.BoundaryType == "baffle" and b.BoundarySubType == "porousBaffle":
                return True
        return False
    ...
    def setupPatchNames(self):
            ...
            if bcSubType == "cyclicAMI":
                ...
                if bc_obj.PeriodicMaster:
                    ...
                    if slave_bc_obj is None:
                        raise ValueError(
                            "No periodic slave boundary linked to master boundary {} was found.".format(
                                bc_obj.Label
                            )
                        )
                    settings["createPeriodics"][bc_obj.Label] = {
                        ...
                        "PeriodicPartner": (
                            slave_bc_obj.Label
                            if bc_obj.PeriodicMaster
                            else slave_bc_obj.PeriodicPartner
                        ),
                        "RotationalPeriodic": slave_bc_obj.RotationalPeriodic,
                        "PeriodicCentreOfRotation": tuple(
                            p for p in slave_bc_obj.PeriodicCentreOfRotation
                        ),
                        "PeriodicCentreOfRotationAxis": tuple(
                            p for p in slave_bc_obj.PeriodicCentreOfRotationAxis
                        ),
                        "PeriodicSeparationVector": tuple(
                            p for p in slave_bc_obj.PeriodicSeparationVector
                        ),
                        "PatchNamesList": '"patch_' + str(bc_id + 1) + '_.*"',
        ...
        for mr_id, mr_obj in enumerate(mr_objs):
            if mr_obj.Extrusion and mr_obj.ExtrusionType == "2DPlanar":
                ...
                settings["createPatches"]["defaultFaces"]["PatchNamesList"] += (
                    ' "patch_0_' + str(mr_id + 1) + '"'
                )
